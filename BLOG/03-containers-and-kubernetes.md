...
>
> **대상 독자**: VM까지는 이해했지만, "컨테이너"와 "쿠버네티스"는 처음 듣는 분.

---

## 컨테이너란 무엇인가?

### 비유: 국제 규격 컨테이너 (해운 컨테이너)

1950년대 이전의 화물 운송을 상상해보자. 항구에 바나나, 자동차 부품, 밀가루가 뒤섞여 있다. 크기도 모양도 다 다르다. 배에 싣는 데만 며칠이 걸리고, 내릴 때도 며칠이 걸린다.

그러다 **표준 컨테이너**가 등장했다. 크기가 정해진 철제 상자다. 무엇을 넣든 상관없이, 컨테이너 자체의 규격이 같으니까 **크레인 하나로 배에 싣고, 기차에 옮기고, 트럭에 실을 수 있다.**

소프트웨어 컨테이너도 똑같은 아이디어다:

| 해운 컨테이너 | 소프트웨어 컨테이너 |
|-------------|------------------|
| 철제 상자 안에 **화물**을 넣는다 | 컨테이너 안에 **앱 + 필요한 라이브러리**를 넣는다 |
| 어떤 배, 어떤 항구든 **규격이 같으면** 운반 가능 | 어떤 서버, 어떤 OS든 **컨테이너 런타임이 있으면** 실행 가능 |
| 컨테이너끼리 **내용물이 섞이지 않는다** | 컨테이너끼리 **파일 시스템과 프로세스가 격리**된다 |

### VM과 컨테이너의 차이

이전 글에서 VM을 "아파트"에 비유했다. 그렇다면 컨테이너는 "원룸"에 가깝다.
...
이 프로젝트에서는 **둘 다** 사용한다:
- **VM**(Tart): 물리 서버 대신 가상 서버를 만드는 데 사용
- **컨테이너**(containerd + K8s): 그 VM 안에서 실제 앱을 돌리는 데 사용

레스토랑 비유로 하면, VM은 **주방 공간 자체**, 컨테이너는 그 주방 안에서 **각 요리사가 맡은 조리대**와 같다. 주방이 먼저 있어야 하고, 그 안에서 조리대를 빠르게 늘리거나 줄일 수 있다.
...
레스토랑으로 비유하면, 요리사가 10명 있을 때:
- **수동**: 사장이 "민수야 1번 테이블, 영희야 3번 테이블..." 일일이 지시
- **쿠버네티스**: **매니저**가 알아서 주문을 분배하고, 요리사가 아프면 다른 사람을 배치하고, 손님이 몰리면 요리사를 추가 투입

쿠버네티스(Kubernetes, 줄여서 K8s)가 바로 그 **매니저**다. 컨테이너를 자동으로 배포하고, 확장하고, 복구하고, 관리하는 시스템이다.
...
쿠버네티스 클러스터는 **Master 노드**와 **Worker 노드**로 나뉜다.

### Master 노드 (Control Plane)

비유: 레스토랑의 **매니저실**

매니저실에는 주문 시스템, CCTV 모니터, 직원 배치표가 있다. 직접 요리는 하지 않지만, 전체 레스토랑이 돌아가게 한다.

Master 노드에서 돌아가는 핵심 프로세스:

| 컴포넌트 | 역할 | 비유 |
|---------|------|------|
| **API Server** | 모든 요청의 입구. kubectl 명령이 여기로 간다. | 접수 창구 |
| **etcd** | 클러스터의 모든 상태 정보 저장소 | 금고 (중요 서류 보관) |
| **Scheduler** | 새 Pod를 어떤 Worker에 배치할지 결정 | 직원 배치 담당자 |
| **Controller Manager** | "원하는 상태"와 "현재 상태"를 계속 비교하고 맞춘다 | 품질 관리자 |

### Worker 노드

비유: 레스토랑의 **주방**

실제로 요리(앱 실행)가 이루어지는 곳이다.

| 컴포넌트 | 역할 | 비유 |
|---------|------|------|
| **kubelet** | Master의 지시를 받아 컨테이너를 실행 | 주방장 (명령 수령) |
| **containerd** | 실제 컨테이너를 만들고 관리 | 조리대 + 기구 |
| **kube-proxy** (또는 Cilium) | 네트워크 트래픽 라우팅 | 서빙 동선 |
...
```
platform 클러스터:
  platform-master  (매니저실)  -- API Server, etcd, Scheduler
  platform-worker1 (주방 1)   -- Prometheus, Grafana, Loki
  platform-worker2 (주방 2)   -- Jenkins, ArgoCD

dev 클러스터:
  dev-master  (매니저실)  -- API Server, etcd, Scheduler
  dev-worker1 (주방)     -- nginx, httpbin, redis, postgres...
...
prod 클러스터:
  prod-master  (매니저실)
  prod-worker1 (주방 1)   -- 이중화
  prod-worker2 (주방 2)   -- 이중화
```
...
레스토랑 비유: 하나의 **조리대**. 메인 셰프(앱 컨테이너)와 보조 셰프(사이드카)가 같은 조리대를 공유한다.
...
레스토랑 비유: "이 메뉴는 항상 조리대 3곳에서 만들어져야 해"라는 **규칙**.
...
레스토랑 비유: **프론트 데스크**. 손님은 프론트 데스크에만 주문하면 되고, 어느 요리사가 만드는지는 몰라도 된다.
...
레스토랑 비유: 같은 건물 안에 **한식 코너, 양식 코너, 디저트 코너**를 나누는 것. 공간은 공유하지만 메뉴와 재료는 구분된다.

---

## kubeadm: 클러스터를 초기화하는 도구

### kubeadm init -- Master 세우기

`kubeadm`은 쿠버네티스 클러스터를 만드는 공식 도구다. Master 노드에서 `kubeadm init`을 실행하면, API Server, etcd, Scheduler, Controller Manager가 모두 설정된다.

### 실제 프로젝트에서는

`scripts/lib/k8s.sh`의 `init_cluster` 함수가 이 과정을 자동화한다:
...
1. platform-master에 SSH 접속
2. kubeadm init 실행
3. 출력되는 join 토큰을 **복사**
4. platform-worker1에 SSH 접속
5. join 토큰 **붙여넣기** 실행
6. platform-worker2에 SSH 접속
7. join 토큰 **붙여넣기** 실행
8. 이것을 dev, staging, prod... **4번 반복**

자동화된 스크립트는 이 모든 것을 루프로 처리한다:
...
다른 클러스터들을 감시하고 관리하는 중앙 관제탑이다. Prometheus가 모든 클러스터의 메트릭을 수집하고, Grafana가 시각화하고, AlertManager가 이상 징후를 알린다. Jenkins가 CI 파이프라인을, ArgoCD가 GitOps 배포를 담당한다.

Worker가 2대인 이유: 모니터링 워크로드(worker1)와 CI/CD 워크로드(worker2)를 분리해서, 모니터링이 과부하 걸려도 CI/CD에 영향을 주지 않게 한다.

**dev -- 실험 주방**

모든 실험이 이루어지는 곳이다. 데모 앱(nginx, httpbin, redis, postgres, rabbitmq, keycloak)이 배포되어 있고, 서비스 메시(Istio), 네트워크 보안 정책(CiliumNetworkPolicy), 오토스케일링(HPA)이 모두 적용되어 있다. 여기서 마음껏 부하 테스트를 하고, 설정을 바꿔보고, 실험할 수 있다.

**staging -- 시식회**

dev에서 검증된 설정을 한 단계 더 확인하는 곳이다. "dev에서 잘 되니까 바로 prod에 올리자"는 위험하다. staging에서 한 번 더 검증한 후 prod에 적용한다.

**prod -- 메인 홀**

실제 서비스가 운영되는 곳이다. 안정성이 최우선이다. Worker 2대로 이중화되어 있어, 한 대가 죽어도 서비스가 유지된다. 실험적인 기능(Istio, 네트워크 정책)은 적용하지 않고, 검증된 설정만 사용한다.

### Pod CIDR이 왜 다른가?

```
platform: pod_cidr = 10.10.0.0/16
dev:      pod_cidr = 10.20.0.0/16
staging:  pod_cidr = 10.30.0.0/16
prod:     pod_cidr = 10.40.0.0/16
```

각 클러스터의 Pod들은 자기만의 IP 대역을 가진다. 왜 다르게 할까?

아파트 비유: A동은 101호~199호, B동은 201호~299호. 호수가 겹치면 택배가 엉뚱한 곳에 배달된다. 마찬가지로, 나중에 클러스터 간 통신을 연결할 때 IP가 겹치면 라우팅이 꼬인다.

---

## Phase 2~5: 노드 준비에서 클러스터 초기화까지

전체 흐름을 정리하면:

```
Phase 1: VM 10대 생성 (이전 글에서 설명)
    |
Phase 2: 노드 준비 (OS 설정)
...
**커널 모듈**: `overlay`는 컨테이너 파일 시스템에 필요하고, `br_netfilter`는 네트워크 브릿지 트래픽을 iptables가 처리할 수 있게 한다.

**ip_forward**: VM이 패킷을 다른 VM으로 전달할 수 있게 해준다. 쿠버네티스 네트워크의 기본 조건이다.
...
```
01. IaC의 필요성    -->  왜 코드로 인프라를 관리하는가?
02. 가상화와 Tart   -->  Mac 한 대에서 VM 10대를 만드는 방법
03. 컨테이너와 K8s  -->  VM 안에서 앱을 효율적으로 운영하는 방법  (지금 여기!)
```

Phase 1~5가 완료되면, 4개의 "빈 클러스터"가 생긴다. 네트워크도, 모니터링도, CI/CD도 아직 없다. 그냥 Master와 Worker가 연결된 상태일 뿐이다.

다음 글부터는 이 빈 클러스터에 하나씩 기능을 입혀간다. 네트워크(Cilium), 모니터링(Prometheus/Grafana), CI/CD(Jenkins/ArgoCD), 오토스케일링(HPA), 서비스 메시(Istio)...

기대하시라.

[<- 이전 글: 02. 가상화란 무엇인가](./02-virtualization.md) | [다음 글: 04. Cilium과 네트워크의 세계 ->](./04-networking.md)
