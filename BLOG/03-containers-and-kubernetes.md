...
4개 클러스터의 노드 수, 리소스 할당, 설치되는 컴포넌트가 모두 다르다. 이는 **각 클러스터의 워크로드 특성과 가용성 요구 수준이 다르기 때문**이다.

- **platform**은 Worker 2대이다. 모니터링(Prometheus, Grafana, Loki)과 CI/CD(Jenkins, ArgoCD)를 별도 Worker에 배치하여 리소스 경합을 방지한다. 모니터링 시스템이 CI/CD 빌드에 의해 리소스 부족으로 죽으면, 장애 감지 자체가 불가능해지기 때문이다.
- **dev**는 Worker 1대로 충분하다. 실험 환경이므로 고가용성이 필요 없고, 리소스를 절약하여 다른 클러스터에 배분하는 것이 합리적이다.
- **staging**도 Worker 1대이다. 배포 검증이 목적이므로 prod의 설정을 재현하되, 이중화까지 복제할 필요는 없다.
- **prod**는 Worker 2대로 이중화한다. 한 Worker가 죽어도 쿠버네티스 Scheduler가 나머지 Worker에 Pod를 재배치하여 서비스 연속성을 보장한다.
...
다른 클러스터들을 감시하고 관리하는 중앙 관제 클러스터다. Prometheus가 모든 클러스터의 메트릭을 수집하고, Grafana가 시각화하고, AlertManager가 이상 징후를 알린다. Jenkins가 CI 파이프라인을, ArgoCD가 GitOps 배포를 담당한다.
...
각 클러스터의 Pod들은 자기만의 IP 대역을 가진다. 왜 다르게 할까? 나중에 클러스터 간 통신을 연결할 때 IP 대역이 겹치면 라우팅 테이블이 충돌하여 패킷이 올바른 목적지로 전달되지 않기 때문이다.
...
[<- 이전 글: 02. 가상화와 VM 관리 (Tart)](./02-virtualization.md) | [다음 글: 04. 네트워킹과 CNI (Cilium) ->](./04-networking.md)
