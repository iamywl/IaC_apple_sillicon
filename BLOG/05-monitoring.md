...
> 시리즈: Apple Silicon 맥에서 구축하는 멀티 클러스터 Kubernetes 인프라 (5/15)
...
이 글에서는 4개 클러스터의 건강 상태를 실시간으로 파악하고, 이상이 생기면 즉시 알림을 보내는 구조를 다룬다.
...
이 프로젝트에서는 학습 목적으로 `webhook-logger`(HTTP echo 서버)를 receiver로 사용한다. 실제 프로덕션에서는:
...
**다음 글에서는** 이 모든 것을 자동으로 구축하는 Infrastructure as Code(IaC) 방법론을 다룬다. Bash 스크립트와 Terraform으로 코드 한 줄로 인프라 전체를 만들고 없애는 방법을 알아본다.
