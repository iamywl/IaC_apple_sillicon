...
**핵심 포인트:**
- DNS 테스트에는 항상 `busybox:1.28` 이미지를 사용 (최신 버전은 nslookup 동작이 다름)
- `--rm`은 Pod 자동 삭제, `--restart=Never`는 Job이 아닌 일반 Pod으로 생성
...
**핵심 포인트:**
- `clusterIP: None`이 Headless Service의 핵심 설정
- 일반 Service: DNS → ClusterIP(1개) 반환
- Headless Service: DNS → 모든 Pod IP 반환
- StatefulSet과 함께 사용하면 `<pod-name>.<service-name>` 형태의 DNS 제공
