...
**시험 출제 패턴:**
- envFrom vs env의 차이 (전체 주입 vs 개별 키 주입)
- Secret 볼륨 마운트 시 각 key가 파일 이름이 됨
- `kubectl create configmap/secret` 빠른 생성 명령어를 외울 것
...
drain:
  - 새 Pod 스케줄링 차단 (cordon 포함)
  - 기존 Pod를 퇴거 (다른 노드로 이동)
```
