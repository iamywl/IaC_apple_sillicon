...
**핵심**: Startup Probe가 성공할 때까지 Liveness Probe는 실행되지 않는다.
...
kubectl get pod exec-probe -w  # RESTARTS 증가 관찰
```
...
```bash
kubectl logs sidecar-log -c log-streamer
```
...
# 3. HTTP 요청
kubectl exec debug-pod -- curl -s localhost:80
```
