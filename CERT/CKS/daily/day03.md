...
### 1.7 imperative 명령어로 RBAC 생성 (시험에서 빠르게 작성)
...
### 1.8 ClusterRole을 RoleBinding으로 범위 제한
...
# ca.crt  namespace  token → 토큰이 마운트되어 있다!

# 토큰 내용 확인 (base64 디코딩된 JWT)
kubectl exec default-pod -n production -- \
  cat /var/run/secrets/kubernetes.io/serviceaccount/token
# eyJhbGciOiJSUzI1NiIs... → JWT 토큰
# 이 토큰으로 API Server에 인증 가능 → 보안 위험!
```
