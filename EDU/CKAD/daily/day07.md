...
helm get values my-nginx               # 사용자가 설정한 값
helm get values my-nginx --all         # 기본값 포함 모든 값
helm get values my-nginx -o yaml       # YAML 형식으로 출력

# Release가 생성한 매니페스트 확인
helm get manifest my-nginx

# Release 정보 확인
helm get all my-nginx                  # 모든 정보 (values + manifest + notes)
```
...
# 특정 Release의 상세 정보
helm history prometheus -n monitoring 2>/dev/null || echo "Release 이름을 helm list로 확인하세요"
