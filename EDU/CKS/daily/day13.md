...
SUID 바이너리 (chmod u+s, 퍼미션 4xxx):
  execve() 시 프로세스의 effective UID = 파일 소유자의 UID로 설정
  → 파일 소유자가 root(UID 0)인 경우, 일반 사용자가 root 권한으로 실행
  → 공격자가 SUID 바이너리의 취약점을 이용하면 로컬 권한 상승(LPE) 가능
...
# 확인
ls -la /usr/bin/newgrp
# -rwxr-xr-x 1 root root ... /usr/bin/newgrp
#    ^-- x = 일반 실행 권한 (SUID 제거됨)
```
...
# k8s:enc:aescbc:v1:key1 접두사 확인 → 암호화 성공!
```
