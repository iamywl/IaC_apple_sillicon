# 02. 가상화와 인프라 — Tart VM과 Apple Silicon
...
2. [하이퍼바이저: 가상화를 가능하게 하는 핵심](#하이퍼바이저-가상화를-가능하게-하는-핵심)
3. [왜 Tart인가 — Apple Virtualization.framework 네이티브](#왜-tart인가--apple-virtualizationframework-네이티브)
4. [골든 이미지 패턴](#왜-골든-이미지-패턴인가)
5. [clusters.json: 10대의 VM이 어떻게 정의되는가](#clustersjson-10대의-vm이-어떻게-정의되는가)
6. [Tart 명령어: VM의 생명주기](#tart-명령어-vm의-생명주기)
7. [VM 관리 코드: scripts/lib/vm.sh](#vm-관리-코드-scriptslibvmsh)
8. [SSH 실행 계층: scripts/lib/ssh.sh](#ssh-실행-계층-scriptslibsshsh)
9. [설정 파싱: scripts/lib/common.sh](#설정-파싱-scriptslibcommonsh)
10. [Phase 1 실행: 01-create-vms.sh](#phase-1-실행-01-create-vmssh)
11. [실제 실행 흐름 따라가기](#실제-실행-흐름-따라가기)
12. [네트워크: VM끼리 어떻게 통신하나?](#네트워크-vm끼리-어떻게-통신하나)
13. [Terraform 인프라](#terraform-인프라)
14. [정리](#정리)
...
**Tart**는 Hypervisor.framework 위에서 동작하는 VM 관리 도구다.
...
- **Master 노드 (2 CPU, 4 GB)**: Master는 API Server, etcd, Scheduler만 실행한다. 실제 애플리케이션 워크로드는 돌리지 않으므로 최소한의 리소스로 충분하다. 4 GB는 etcd와 apiserver가 안정적으로 동작하기 위한 최소 권장 사양이다.
...
| IP 대기 시간 변경 | `vm_wait_for_ip()`의 재시도 횟수와 `sleep` 간격 |
...
이 프로젝트는 Tart 명령어를 직접 치지 않는다. 대신 `scripts/lib/vm.sh`에 함수로 감싸두었다.
...
`clusters.json`을 jq로 파싱하여 다른 스크립트에 제공한다.
...
Terraform은 Bash 스크립트와 동일한 작업을 선언적으로 수행한다.
...
SSH로 각 노드에 접속하여 kubeadm 설치를 수행한다.
`local-exec` provisioner 안에서 Bash 스크립트를 실행한다.

### helm-releases 모듈 (terraform/modules/helm-releases/main.tf)

Terraform Helm provider를 사용하여 차트를 배포한다.
...
둘 다 같은 인프라를 만든다. 필요에 따라 선택하면 된다.
...
| **ssh.sh** | VM 내부 명령 실행을 위한 SSH 래퍼 함수 라이브러리 |
| **common.sh** | clusters.json 파싱 및 로깅 헬퍼 함수 라이브러리 |
| **Terraform** | 선언적 인프라 관리 도구 (Bash 스크립트의 대안) |

---

[← 이전: 01. 프로젝트 개요](./01-project-overview.md) | [다음: 03. Kubernetes 설치 →](./03-kubernetes-setup.md)
