...
2. [CI/CD가 뭔가요?](#cicd가-뭔가요)
   - [왜 CI/CD가 필요한가](#왜-cicd가-필요한가)
3. [Jenkins -- 파이프라인의 엔진](#jenkins----파이프라인의-엔진)
   - [Jenkins가 뭔가요?](#jenkins가-뭔가요)
   - [왜 Jenkins인가](#왜-jenkins인가)
   - [Pipeline이라는 개념](#pipeline이라는-개념)
4. [ArgoCD -- GitOps의 실현](#argocd----gitops의-실현)
   - [ArgoCD가 뭔가요?](#argocd가-뭔가요)
   - [왜 ArgoCD인가](#왜-argocd인가)
   - [GitOps 개념: Git = 진실의 단일 원천](#gitops-개념-git--진실의-단일-원천-single-source-of-truth)
5. [실제 프로젝트의 ArgoCD 설정](#실제-프로젝트의-argocd-설정)
   - [prune와 selfHeal이 왜 중요한가?](#prune와-selfheal이-왜-중요한가)
6. [7단계 Jenkins 파이프라인 상세 분석](#7단계-jenkins-파이프라인-상세-분석)
   - [Stage 1: Validate Manifests](#stage-1-validate-manifests-매니페스트-검증)
   - [Stage 2: Security Scan](#stage-2-security-scan-보안-검사)
   - [Stage 3: Deploy to Dev](#stage-3-deploy-to-dev-argocd로-배포)
   - [Stage 4: Wait for Rollouts](#stage-4-wait-for-rollouts-롤아웃-대기)
   - [Stage 5: Health Check](#stage-5-health-check-상태-확인)
   - [Stage 6: Integration Test](#stage-6-integration-test-통합-테스트)
   - [Stage 7: Smoke Test](#stage-7-smoke-test-전체-체인-e2e-검증)
7. [파이프라인이 실패하면?](#파이프라인이-실패하면)
8. [전체 흐름 정리](#전체-흐름-정리)
9. [실제 프로젝트에서는](#실제-프로젝트에서는)
10. [핵심 요약](#핵심-요약)
11. [관련 파일](#관련-파일)
...
> **다음 글**: [08. 오토스케일링](08-autoscaling.md)
>
> 파이프라인의 7번째 단계에서 "L7 정책이 POST를 차단하는지" 확인했다.
> 네트워크 보안 정책에 대해서는 [04. 네트워킹](04-networking.md)에서 다룬다.
