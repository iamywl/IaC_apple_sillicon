...
2. [6단계 디버깅 프레임워크](#6단계-디버깅-프레임워크)
3. [레이어별 디버깅 체크리스트](#레이어별-디버깅-체크리스트)
   - [Layer 0: VM이 시작되지 않을 때](#layer-0-vm이-시작되지-않을-때)
   - [Layer 1: SSH 접속이 안 될 때](#layer-1-ssh-접속이-안-될-때)
   - [Layer 2: K8s 클러스터에 문제가 있을 때](#layer-2-k8s-클러스터에-문제가-있을-때)
   - [Layer 3: Pod에 문제가 있을 때](#layer-3-pod에-문제가-있을-때)
   - [Layer 4: 서비스에 접속이 안 될 때](#layer-4-서비스에-접속이-안-될-때)
   - [Layer 5: 네트워크 정책으로 트래픽이 차단될 때](#layer-5-네트워크-정책으로-트래픽이-차단될-때)
4. [자주 발생하는 에러와 해결법 모음](#자주-발생하는-에러와-해결법-모음)
5. [이 프로젝트에서 발생한 실제 버그들](#이-프로젝트에서-발생한-실제-버그들)
6. [유용한 진단 명령어 모음](#유용한-진단-명령어-모음)
7. [재해 복구 절차](#재해-복구-절차)
8. [트러블슈팅 마인드맵](#트러블슈팅-마인드맵)
...
이번 문서는 이 프로젝트의 `docs/bug-reports/`에 기록된 실제 경험을 바탕으로 한다.
...
Google SRE 팀은 이 프레임워크를 **Postmortem(사후 분석)**이라고 부른다. 장애가 발생하면 반드시 문서를 작성하고, "누가 잘못했나"가 아니라 "시스템을 어떻게 개선할 수 있나"에 초점을 맞춘다. 이 프로젝트의 `docs/bug-reports/` 디렉토리가 바로 이 Postmortem의 축소판이다.
...
| CPU/메모리 초과 할당 | VM 시작 실패 | `clusters.json`의 리소스 합계 확인 (총 21 vCPU / 66GB) |
...
이 프로젝트의 `docs/learning/troubleshooting.md`가 바로 이 런북의 역할을 한다.
