...
15편에 걸쳐 Apple Silicon Mac 한 대에서 **프로덕션급 멀티클러스터 Kubernetes 인프라**를 만들었다. 이번 마지막 편에서는 모든 것을 한데 모아 정리한다.
...
| Phase 13 | 시크릿 관리 | Sealed Secrets 컨트롤러 + RBAC |
| Phase 14 | 정책 강제 | RBAC + OPA Gatekeeper 제약 조건 |
| Phase 15 | 백업/DR | etcd 스냅샷 + Velero 리소스 백업 |
| Phase 16 | 리소스 관리 | ResourceQuota + LimitRange |
| Phase 17 | 이미지 레지스트리 | Harbor 프라이빗 컨테이너 레지스트리 |
...
| 편 | 주제 | 핵심 기술 |
|----|------|----------|
| 11 | 부하 테스트 | k6, stress-ng, VU, p95/p99, RPS |
| 12 | 트러블슈팅 | 6단계 프레임워크, 레이어별 디버깅 |
...
15편에 걸쳐 **빈 Mac에서 프로덕션급 멀티클러스터 인프라**를 만들었다. 돌아보면:
