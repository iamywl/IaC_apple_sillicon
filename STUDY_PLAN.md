...
> **전제**: 하루 2시간 학습 | **총 소요**: 약 6주 (30일, 총 60시간)
...
| **6주차** | 통합 실습 & 마무리 | Day 26~30 | 전체 구축 재현, Terraform 대안 |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1h | 이론: 인프라 엔지니어링의 정의와 12단계 파이프라인 전체 흐름을 학습한다. | `BLOG/01-introduction.md` |
| 30m | 심화: 프로젝트 전체 아키텍처를 파악한다. | `EDU/01-project-overview.md` |
| 30m | 실습: 필수 도구를 설치하고 시스템 요구사항을 점검한다. | `GUIDE/00-prerequisites.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1h | 이론: Hypervisor 종류, 커널 격리, Golden Image 개념을 학습한다. | `BLOG/02-virtualization.md` |
| 30m | 심화: Tart VM과 Terraform 인프라 계층을 학습한다. | `EDU/02-infrastructure.md` |
| 30m | 실습: 4 클러스터 구조와 네트워크 설계를 확인한다. | `GUIDE/01-architecture.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1.5h | 이론: containerd, CRI, Pod, Deployment, kubeadm을 학습한다. | `BLOG/03-containers-and-kubernetes.md` |
| 30m | 심화: 12단계 설치 스크립트의 구조를 파악한다. | `EDU/03-kubernetes-setup.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1h | 실습: demo.sh로 전체 환경을 구축해본다. | `GUIDE/02-quick-start.md` |
| 1h | 실습: 12단계 설치 파이프라인 중 Phase 1~6의 동작을 확인한다. | `GUIDE/03-phase-by-phase.md` (Phase 1~6) |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1h | 실습: 노드, Pod, 서비스가 정상 동작하는지 확인한다. | `GUIDE/04-cluster-verification.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1.5h | 이론: CNI, eBPF, Pod CIDR, NAT 없는 통신을 학습한다. | `BLOG/04-networking.md` |
| 30m | 심화: Cilium CNI와 Hubble의 구조를 파악한다. | `EDU/04-networking.md` (전반부) |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1h | 심화: Istio와 네트워크 정책을 상세히 학습한다. | `EDU/04-networking.md` (후반부) |
| 1h | 실습: Phase 7~12 설치 파이프라인 중 네트워크 관련 Phase를 확인한다. | `GUIDE/03-phase-by-phase.md` (Phase 7~12) |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 2h | 이론: Default Deny, L3/L4/L7 필터링, CiliumNetworkPolicy를 학습한다. | `BLOG/08-network-security.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 2h | 이론: Sidecar 패턴, mTLS, VirtualService, 카나리 배포를 학습한다. | `BLOG/10-service-mesh.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1h | 실습: Hubble로 네트워크 플로우를 직접 관찰한다. | `GUIDE/07-testing-scenarios.md` (네트워크 부분) |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1.5h | 이론: 옵저버빌리티 Three Pillars(Metrics/Logs/Traces)와 Prometheus Pull 모델을 학습한다. | `BLOG/05-monitoring.md` |
| 30m | 심화: Prometheus와 Grafana의 내부 구조를 파악한다. | `EDU/05-monitoring.md` (전반부) |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1h | 심화: Loki, AlertManager, 알림 라우팅을 학습한다. | `EDU/05-monitoring.md` (후반부) |
| 1h | 실습: Grafana 대시보드에 접속하여 메트릭을 직접 확인한다. | `GUIDE/05-dashboard-guide.md` (모니터링 부분) |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 2h | 이론: Bash vs Terraform, 명령형 vs 선언형, 멱등성, SSOT를 학습한다. | `BLOG/06-iac-automation.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 2h | 실습: Terraform 모듈로 동일한 인프라를 구축해본다. | `GUIDE/09-terraform-alternative.md` |
...
| 1h | 실습: Prometheus PromQL 쿼리를 연습하고, Terraform 코드를 읽어본다. | `manifests/`, `terraform/` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 2h | 이론: Jenkins 7단계 파이프라인과 ArgoCD GitOps를 학습한다. | `BLOG/07-cicd-pipeline.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 2h | 실습: Jenkins + ArgoCD 파이프라인을 직접 구성하고 동작을 확인한다. | `GUIDE/06-cicd-workflow.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1.5h | 이론: metrics-server, HPA 공식, PDB를 학습한다. | `BLOG/09-autoscaling.md` |
| 30m | 실습: HPA가 실제로 동작하는 것을 관찰한다. | `GUIDE/07-testing-scenarios.md` (스케일링 부분) |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1h | 이론: SRE 대시보드의 구조(6 Pages, 11 APIs)를 학습한다. | `BLOG/12-sre-dashboard.md` |
| 1h | 심화: React + Express 코드 구조를 분석한다. | `EDU/06-dashboard.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1h | 실습: 대시보드를 설치하고 직접 사용해본다. | `GUIDE/05-dashboard-guide.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1.5h | 이론: 3-Tier 아키텍처, OAuth 2.0, RabbitMQ를 학습한다. | `BLOG/11-demo-apps.md` |
| 30m | 심화: 데모 앱과 HPA 설정을 상세히 분석한다. | `EDU/07-demo-apps.md` (전반부) |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1.5h | 이론: k6, VU(Virtual Users), p95/p99, RPS, SLA를 학습한다. | `BLOG/13-load-testing.md` |
| 30m | 심화: stress-ng와 Cascade 시나리오를 학습한다. | `EDU/07-demo-apps.md` (후반부) |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 2h | 실습: 16개 시나리오 중 주요 시나리오를 직접 실행한다. | `GUIDE/07-testing-scenarios.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1h | 이론: 6단계 디버깅 방법론과 레이어별 체크리스트를 학습한다. | `BLOG/14-troubleshooting.md` |
| 1h | 실습: 계층별 문제 진단을 직접 수행한다. | `GUIDE/08-troubleshooting.md` |
...
| 1h | 실습: Pod를 삭제하여 장애를 시뮬레이션하고 복구 과정을 관찰한다. | `GUIDE/07-testing-scenarios.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 2h | 학습: Day 0/1/2 운영 개념, 커리어 패스, 치트시트를 정리한다. | `BLOG/15-putting-it-all-together.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 1h | 학습: 새 클러스터, 앱, 기능을 추가하는 방법을 익힌다. | `EDU/08-how-to-modify.md` |
| 1h | 학습: 전체 기술 스택 레퍼런스를 확인한다. | `EDU/09-tech-stack.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 2h | 통합 실습: 클린 환경에서 demo.sh로 전체 인프라 구축을 시작한다. | `GUIDE/02-quick-start.md` + `GUIDE/03-phase-by-phase.md` |
...
| 시간 | 활동 | 자료 |
|------|------|------|
| 2h | 통합 실습: 클러스터 검증, 대시보드 설치, 부하 테스트까지 완료한다. | `GUIDE/04-cluster-verification.md` ~ `GUIDE/07-testing-scenarios.md` |
...
| 자료 | 총 분량(추정) | 학습 시점 | 용도 |
|------|-------------|----------|------|
| **BLOG 01~03** | 3.5h | Day 1~3 | 기초 이론이다. |
| **BLOG 04, 08, 10** | 5.5h | Day 6~9 | 네트워크/보안 이론이다. |
| **BLOG 05~06** | 3.5h | Day 11~13 | 모니터링/IaC 이론이다. |
| **BLOG 07, 09, 12** | 4.5h | Day 16~19 | CI/CD, 스케일링, 대시보드 이론이다. |
| **BLOG 11, 13~15** | 7h | Day 21~26 | 앱, 테스트, 트러블슈팅, 정리이다. |
| **EDU 01~03** | 1.5h | Day 1~3 | 기초 심화이다. |
| **EDU 04~05** | 2h | Day 6~12 | 네트워크/모니터링 심화이다. |
| **EDU 06~07** | 2h | Day 19~22 | 대시보드/앱 심화이다. |
| **EDU 08~09** | 2h | Day 27 | 확장/레퍼런스이다. |
| **GUIDE 00~04** | 5h | Day 1~5 | 환경 구축 실습이다. |
| **GUIDE 05~09** | 8h | Day 12~29 | 운영, 테스트, 대안 실습이다. |
...
- `manifests/`와 `terraform/` 디렉토리의 실제 파일을 함께 열어보는 것이 중요하다.
