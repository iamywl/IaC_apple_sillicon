...
> ## ⛔ 다시 강조 — Codex 는 "실행"도 "캡처"도 하지 않는다
> Codex 의 본질은 **작성된 문서 자체를 읽고 리뷰하는 것**이다. 클러스터 명령(`kubectl`/`tart`/SSH)·스크린샷 캡처·교재 편집은 **절대** 하지 않는다(환경에 없을뿐더러 역할이 아니다). 아래에 `grep`·`python3` 같은 **읽기 전용 텍스트 검색**을 "보조 수단"으로 적어 두지만, 이는 어디까지나 문서를 더 빠르게 훑기 위한 선택지일 뿐이다. **이런 정적 도구를 못 쓰는 환경이라면 그냥 사람처럼 파일을 하나씩 열어 읽고 리뷰하면 된다.** 핵심 산출물은 언제나 "읽고 판단한 결과를 정리한 피드백 표"다.
