...
초기 분산 시스템에서는 ZooKeeper 같은 도구로 클러스터 상태를 관리했으나, ZooKeeper는 Java 기반으로 무겁고 운영이 복잡했다. CoreOS 팀이 Go 언어로 경량화하고, Raft 합의 알고리즘을 채택하여 만든 것이 etcd이다. 쿠버네티스는 설계 초기부터 etcd를 유일한 상태 저장소로 채택하여, 모든 컴포넌트가 etcd의 상태를 기준으로 동작하는 아키텍처를 구성했다. etcd가 죽으면 클러스터의 모든 제어 기능이 정지하므로, etcd 백업/복구는 클러스터 운영에서 가장 중요한 작업이다.
...
쿠버네티스는 약 4개월마다 새 마이너 버전을 릴리스한다. 보안 패치, 버그 수정, 새로운 기능을 적용하려면 클러스터 업그레이드가 필수이다. 그러나 버전 간 API 호환성 문제가 있으므로 한 번에 여러 버전을 건너뛸 수 없다. kubeadm은 이 복잡한 업그레이드 과정을 자동화하여, Static Pod 매니페스트의 이미지 태그 변경, 인증서 갱신, 애드온 업데이트 등을 일괄 처리한다.
...
```text
# 정상: Running 상태의 etcd 컨테이너가 보여야 한다
...
default     web-app-pdb  2               N/A                0                    5d
```
