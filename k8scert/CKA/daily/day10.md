...
health-check   */5 * * * *   False     0        <none>          10s
```
...
Used가 모두 0인 것은 아직 리소스를 사용하는 Pod가 없기 때문이다. 이 네임스페이스에서 Pod를 생성하려면 반드시 resources.requests/limits를 명시해야 한다(quota에 CPU/memory가 포함되어 있으므로).
...
cache-pod   1/1     Running   0          30s   10.20.1.10    prod-worker1
web-pod     1/1     Running   0          10s   10.20.1.11    prod-worker1
```
...
# 비정상: 출력 없음 → metrics-server가 설치되지 않은 것
```
