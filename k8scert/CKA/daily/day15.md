...
PV Controller(kube-controller-manager 내부)는 PVC 생성 이벤트를 Watch한다. 새로운 PVC가 생성되면 Controller는 Available 상태의 모든 PV를 순회하며 바인딩 조건을 확인한다. 조건이 일치하는 PV를 찾으면 PV의 `spec.claimRef`에 PVC 정보를 기록하고 PVC의 `spec.volumeName`에 PV 이름을 기록하여 양방향 바인딩을 완성한다. Dynamic Provisioning의 경우 StorageClass에 정의된 provisioner가 실제 스토리지를 생성하고 PV 객체를 자동으로 만든다.
...
**등장 배경:** Immediate 모드에서는 PVC 생성 즉시 PV가 프로비저닝된다. 로컬 스토리지(hostPath, local PV)의 경우 특정 노드에 PV가 생성되면 Pod가 반드시 그 노드에 스케줄링되어야 한다. 이는 nodeAffinity, resource request 등 다른 스케줄링 조건과 충돌할 수 있다. WaitForFirstConsumer는 Pod가 실제로 스케줄링될 때까지 PV 프로비저닝을 지연시켜 이 문제를 해결한다.
