...
**핵심:** CrashLoopBackOff는 `kubectl logs --previous`로 이전 컨테이너의 로그를 확인하는 것이 핵심이다. `--previous` 플래그 없이 `kubectl logs`를 실행하면 현재 컨테이너(재시작 직후)의 빈 로그만 보이므로 원인을 파악할 수 없다. 컨테이너가 재시작될 때 이전 로그는 kubelet의 로그 저장소에 보존되며, `--previous`로 이를 조회한다.
...
```text
NAME               READY   STATUS    RESTARTS   AGE
broken-image-pod   1/1     Running   0          15s
```
