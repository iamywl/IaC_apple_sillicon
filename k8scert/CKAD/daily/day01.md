...
**내부 동작 원리 심화:**
Pod 생성 시 kubelet은 먼저 pause 컨테이너(infra container)를 생성한다. pause 컨테이너는 네트워크 네임스페이스를 보유하는 유일한 컨테이너로, 나머지 앱 컨테이너들이 이 네임스페이스에 합류(join)한다. pause 컨테이너가 존재하는 이유는 앱 컨테이너가 재시작되더라도 네트워크 네임스페이스(IP, 포트 바인딩)가 유지되도록 하기 위함이다. CNI 플러그인은 pause 컨테이너의 네임스페이스에 veth pair를 생성하고 Pod IP를 할당한다.
...
**등장 배경:**
멀티스테이지 빌드 이전에는 빌드 환경과 실행 환경을 분리하려면 두 개의 Dockerfile을 유지하거나, 빌드 스크립트에서 docker cp로 아티팩트를 추출한 뒤 별도 이미지를 빌드하는 "빌더 패턴"을 사용해야 했다. 이 방식은 CI 파이프라인이 복잡해지고, 빌드 컨텍스트 관리가 어렵다. Docker 17.05에서 도입된 멀티스테이지 빌드는 단일 Dockerfile 안에서 이 모든 것을 해결한다.
...
```text
{"exitCode":1,"reason":"Error","startedAt":"2026-03-30T00:00:00Z","finishedAt":"2026-03-30T00:00:01Z"}
```
...
```text
Events:
  Warning  Failed   10s   kubelet  Failed to pull image "nginx:invalid": rpc error: code = NotFound
  Warning  Failed   10s   kubelet  Error: ErrImagePull
  Normal   BackOff  8s    kubelet  Back-off pulling image "nginx:invalid"
...
ckad-test   1/1     Running   0          10s   10.244.x.x    dev-node-1   ...
```
...
nginx   NodePort   10.96.x.x     <none>        80:30080/TCP   xxd
```
