...
**등장 배경:**
Pod를 직접 생성하면 Pod가 삭제되었을 때 자동 복구가 되지 않는다. ReplicaSet은 Pod 수를 유지해 주지만, 이미지 업데이트 시 이전 RS를 수동으로 축소하고 새 RS를 생성해야 한다. Deployment는 ReplicaSet 위에 추가 추상화 계층을 제공하여, 업데이트 전략(RollingUpdate, Recreate), 롤백, 일시 정지 등을 선언적으로 관리한다. 쿠버네티스에서 stateless 워크로드를 배포하는 표준 방법이다.
...
**내부 동작 원리 심화:**
Deployment Controller는 spec.template의 해시값(pod-template-hash)을 계산하여 ReplicaSet 이름의 접미사로 사용한다. template이 변경되면 새 해시가 생성되고, 이에 대응하는 ReplicaSet이 없으면 새로 생성한다. 이전 ReplicaSet은 replicas=0으로 유지되어 롤백 시 재사용된다. `revisionHistoryLimit`(기본 10)으로 보관할 이전 RS 수를 제한한다. spec.template 외의 변경(replicas, strategy 등)은 새 ReplicaSet을 생성하지 않는다.
...
web-deploy-abc123def    3         3         3       30s
```
...
- **Readiness Probe 실패**: 새 Pod가 Ready 상태가 되지 않아 롤아웃이 진행되지 않는다.
...
```text
The Deployment "web-deploy" is invalid: spec.template.metadata.labels: Invalid value: ... `selector` does not match template `labels`
```
...
```text
NAME                      DESIRED   CURRENT   READY   AGE
web-deploy-abc123         3         3         3       10m
web-deploy-def456         0         0         0       8m
web-deploy-ghi789         0         0         0       6m
web-deploy-jkl012         0         0         0       4m
...
```
...
nginx-web   1/1     1            1           xxd   nginx        nginx:1.25   app=nginx-web
```
...
nginx-web-xxxxxxxxx-aaaaa   1/1     Running   0          xxd
nginx-web-xxxxxxxxx-bbbbb   1/1     Running   0          5s
```
