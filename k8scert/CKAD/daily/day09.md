...
```text
    ...
    Liveness:       http-get http://:80/healthz delay=5s timeout=3s period=10s #success=1 #failure=3
    Readiness:      http-get http://:80/ready delay=5s timeout=1s period=5s #success=1 #failure=3
```
...
기대 출력:
```text
Mon Jan 15 10:30:05 UTC 2024 [INFO] Processing request...
Mon Jan 15 10:30:10 UTC 2024 [INFO] Processing request...
Mon Jan 15 10:30:15 UTC 2024 [INFO] Processing request...
```
...
Distroless 이미지, scratch 기반 이미지에는 셸(/bin/sh)이 없다.
kubectl exec가 불가능하므로 디버깅 수단이 없었다.

Kubernetes 1.23에서 Ephemeral Container가 GA 되면서,
실행 중인 Pod에 임시 디버그 컨테이너를 주입할 수 있게 되었다.
Ephemeral Container는 Pod spec에 영구적으로 추가되지 않으며,
Pod 재시작 후에는 사라진다.
```
...
slow-app     1/1     Running   15 (30s ago)  10m
```
