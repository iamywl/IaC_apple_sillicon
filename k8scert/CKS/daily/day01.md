...
3. CNI 플러그인(Cilium/Calico)의 Controller가 감지
   ...
   │   └─ TC(Traffic Control) hook 또는 XDP hook에 BPF 프로그램을 attach한다
   │   └─ 커널 네트워크 스택 진입 전에 패킷을 판정하므로 오버헤드가 낮다
   │
   └─ Calico: iptables/ipvs 규칙으로 변환
       └─ 각 노드의 kube-proxy와 협력
       └─ netfilter FORWARD 체인에 iptables 규칙을 삽입한다
   │
   ▼
4. 각 노드의 커널에서 패킷 단위로 허용/차단 판단
   │
   ├─ 허용: 패킷이 목적지 Pod로 전달
   └─ 차단: 패킷 드롭 (타임아웃, RST 없음 — 공격자에게 정보 노출 방지)

커널 수준 동작 원리:
  - Cilium(eBPF): sk_buff 구조체에서 src/dst IP, port, protocol을 추출하고
    eBPF map에 저장된 정책 엔트리와 매칭한다. O(1) 해시 룩업으로 판정한다.
  - Calico(iptables): netfilter 프레임워크의 FORWARD 체인에서 -j DROP 또는
    -j ACCEPT 규칙으로 패킷을 처리한다. 규칙 수에 비례하는 O(n) 선형 탐색이다.
...
1234       Enabled            Enabled           12345      k8s:app=web
```

### 1.12 NetworkPolicy 트러블슈팅
...
notAfter=Mar 30 00:00:00 2027 GMT
```
