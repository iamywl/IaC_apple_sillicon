...
```text
secure-ns   Active   pod-security.kubernetes.io/enforce=restricted,pod-security.kubernetes.io/enforce-version=latest
```
...
```text
Warning  FailedCreate  replicaset/nginx-xxxx  Error creating: pods "nginx-xxxx" is forbidden: violates PodSecurity "restricted:latest"
```
...
```text
Error: error opening encryption provider configuration file: open /etc/kubernetes/encryption-config.yaml: no such file or directory
```
