...
검증 기대 출력:
```text
Warning Shell in container (user=root container=nginx shell=sh parent=runc cmdline=sh -c echo test image=nginx pod=nginx-xxxx ns=demo)
```
...
● falco.service - Falco: Container Native Runtime Security
     Loaded: loaded (/usr/lib/systemd/system/falco.service; enabled)
...
local.yaml이 목록에 없으면 추가해야 한다. 순서가 중요하다 — local.yaml이 뒤에 있어야 기본 룰을 오버라이드한다.
...
원인: NetworkPolicy는 kubectl exec 트래픽(kubelet ↔ Pod)을 차단하지 않는다.
      실제 원인은 다른 곳에 있다 — kubectl exec는 kubelet이 직접 컨테이너에
      연결하므로 NetworkPolicy 영향을 받지 않는다.
해결: kubectl exec 타임아웃은 kubelet 문제이다. kubectl exec가 필요하면
      NetworkPolicy와 무관하게 동작한다.
```
