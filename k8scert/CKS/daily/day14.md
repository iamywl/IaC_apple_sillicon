...
```
[일반 컨테이너 — runc]
  컨테이너 프로세스 → syscall → 호스트 커널 (Linux namespace로만 격리)
  격리 약함: 커널 취약점(예: CVE-2022-0185 fsconfig 취약점)으로
  namespace를 탈출하여 호스트를 장악할 수 있다

[gVisor 컨테이너 — runsc]
  컨테이너 프로세스 → syscall → gVisor Sentry(사용자 공간 커널)
  Sentry가 syscall을 직접 구현하거나, 안전한 것만 호스트 커널에 전달한다
  약 300개의 Linux syscall 중 ~70%를 Sentry가 자체 처리하며,
  나머지는 Gofer 프로세스를 통해 제한된 방식으로 호스트에 전달한다

  구조:
  ┌─────────────────────────────────────────┐
  │ 컨테이너 프로세스 (앱)                    │
  │     │ syscall (ptrace/KVM 인터셉트)      │
  │     ▼                                   │
  │ Sentry (Go로 작성된 사용자 공간 커널)      │
  │     │ 파일 I/O만 Gofer에 위임            │
  │     ▼                                   │
  │ Gofer (파일시스템 프록시, 최소 권한)        │
  └─────────────────────────────────────────┘
       │ 제한된 syscall만 호스트 커널에 전달
       ▼
  호스트 Linux 커널
```
...
검증 기대 출력:
```text
Error from server (Forbidden): pods "test-image" is forbidden: image policy webhook backend denied one or more images: evil-registry.com/malware:latest
```
...
isolate-quarantine       quarantine=true    30s
```
...
```text
-rw-r--r-- 1 root root 4523 Mar 30 10:15 /tmp/incident-evidence.txt
```
...
a1b2c3d4    xxxx    10 seconds ago   Running   kube-apiserver    0
```
...
```text
# volume mount 경로 오류
Error: open /etc/kubernetes/audit-policy.yaml: no such file or directory

# YAML 문법 오류
Error: error decoding: json: cannot unmarshal...

# 플래그 값 오류
Error: unknown flag: --encryption-provider-confg  (오타!)
```
...
기대 출력 (오류 시):
```text
FATAL Runtime error: Could not load rules file /etc/falco/falco_rules.local.yaml: yaml-cpp: error at line 5, column 3
```
