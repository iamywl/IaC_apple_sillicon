...
> 명령 실행 결과는 실제 터미널 스크린샷(§4①) 또는 미캡처/참조 노트로 제공한다(출력을 텍스트 블록으로 지어내지 않는다).
> **노드 이름 표기:** 본문의 `node01` 등은 일반 클러스터/시험 환경 기준 이름이다. 로컬 재현 시 `node01`→`dev-worker1`로 읽고 `ssh dev-master`처럼 VM 별칭으로 접속한다.
...
_그림. etcd 3노드 구성. Leader 가 쓰기를 받아 Follower 에 복제하고, 과반수 합의로 확정한다._

**장애 허용 계산** (공식: 장애 허용 = (n−1)/2, n=총 노드 수, 홀수 권장):

| 노드 수 | quorum(정족수) | 장애 허용 |
|:--:|:--:|:--:|
| 3 | 2 | 1 |
| 5 | 3 | 2 |
| 7 | 4 | 3 |

**Raft 합의 과정(쓰기 요청):** ① 클라이언트가 Leader 에 쓰기 요청 → ② Leader 가 Follower 에 로그 복제 → ③ 과반수(quorum) 응답 시 commit 확정 → ④ Leader 가 클라이언트에 성공 응답.
