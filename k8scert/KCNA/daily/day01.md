...
Kubernetes 이전에는 컨테이너 오케스트레이션에 표준이 없었다. Docker Swarm, Apache Mesos, Cloud Foundry 등 여러 도구가 경쟁했으나, 각각 한계가 있었다.
...
Docker Swarm:
  - Docker와 긴밀하게 통합되어 있으나 기능이 제한적
  - 대규모 클러스터에서 성능/안정성 부족
  - 선언적 오브젝트 모델이 미흡

Apache Mesos:
  - 범용 리소스 스케줄러로 컨테이너 전용이 아님
  - 학습 곡선이 가파르고 운영 복잡도가 높음
  - Marathon 프레임워크를 별도로 사용해야 했음
...
```text
NAME                                      IMAGE
etcd-platform-control-plane               registry.k8s.io/etcd:3.5.10-0
kube-apiserver-platform-control-plane     registry.k8s.io/kube-apiserver:v1.29.0
kube-controller-manager-platform-...      registry.k8s.io/kube-controller-manager:v1.29.0
kube-scheduler-platform-control-plane     registry.k8s.io/kube-scheduler:v1.29.0
```
...
kube-proxy-xxxxx   1/1     Running   0          7d    192.168.64.2    platform-control-plane
```
