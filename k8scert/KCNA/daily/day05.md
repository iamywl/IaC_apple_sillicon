...
> 난이도: ★★★★☆
>
> **전제(Day 4 연결):** Day 4 에서 배운 ConfigMap/Secret 은 컨테이너 `spec` 에 연결되어 설정을 외부화한다. 오늘은 그 설정을 담아 실행하는 **컨테이너 자체의 내부 기술 기반**(namespace·cgroups·OCI)과, 이 컨테이너들을 다중 노드에서 조율하는 **오케스트레이션** 개념을 이해한다.
