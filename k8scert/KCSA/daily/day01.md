...
```
기존 방식의 한계:
전통적 보안은 "경계 보안(Perimeter Security)"에 의존했다. 방화벽 하나로 내부와 외부를
구분하고, 내부는 무조건 신뢰하는 구조였다. 그러나 이 모델은 다음과 같은 문제를 가진다:

1. 내부 침해 시 횡적 이동(Lateral Movement)이 자유롭다
2. 마이크로서비스 환경에서 수백 개의 서비스가 네트워크로 통신하므로 공격 표면이 확대된다
3. Pod가 동적으로 생성/삭제되어 IP 기반 보안 규칙이 무용지물이 된다
4. 컨테이너 오케스트레이션 환경에서 공유 인프라의 격리가 필수적이다

해결:
클라우드 네이티브 보안은 "심층 방어(Defense in Depth)" 원칙을 채택한다.
4C 모델로 각 계층에 독립적인 보안 통제를 배치하여, 하나의 계층이 침해되더라도
다른 계층이 추가 방어선을 형성하는 구조다.
```
...
5
CiliumNetworkPolicy 수 (Zero Trust 구현)
nginx-... runAsNonRoot=true
httpbin-... runAsNonRoot=true
```
