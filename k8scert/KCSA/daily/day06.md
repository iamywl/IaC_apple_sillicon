...
공격-방어 매핑:
- Lateral Movement(MITRE ATT&CK) → NetworkPolicy로 횡적 이동 차단
- Information Disclosure(STRIDE) → 네트워크 수준에서 접근 제한
...
NAME                         AGE
allow-dns                    30d
allow-httpbin-from-nginx     30d
allow-nginx-ingress          30d
default-deny-all             30d
```
...
**검증 — 기대 출력:**
```text
=== PSA Restricted 적용 시뮬레이션 ===
Warning: existing pods in namespace "demo" violate the new PodSecurity enforce level "restricted:latest"

=== 현재 demo NS 레이블 ===
NAME   STATUS   AGE   LABELS
demo   Active   30d   kubernetes.io/metadata.name=demo
```
...
장애 시나리오 1: Egress default-deny 후 DNS 해석 실패
  ...
  디버깅:
    ...
    kubectl get networkpolicies -n demo -o yaml | grep -A5 egress
  해결: DNS 허용 정책을 생성한다:
    egress:
    - to:
        - namespaceSelector: {}
