...
```
기존 방식의 한계:
전통적인 보안 프레임워크(방화벽 규칙, 안티바이러스)는
"알려진 위협 시그니처"에 의존했다. 컨테이너 환경에서는:

1. 공격자의 전술/기법이 컨테이너에 특화되어 있어 기존 탐지 규칙이 무용하다
2. "무엇을 방어해야 하는가?"에 대한 체계적인 분류가 없었다
3. 보안 팀이 공격자의 사고 흐름(Kill Chain)을 이해하지 못하면
   방어 전략이 단편적으로 된다
...
[Initial Access 방어]
  API Server 접근 제한: 1 (1=활성화)

[Credential Access 방어]
  etcd TLS: 1 (1=활성화)

[Execution 방어]
  PSA 레이블: 0
```
PSA 레이블이 0이면 PSA가 미적용된 상태이므로 보안 강화가 필요하다.
